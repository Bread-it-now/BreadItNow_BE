...
import com.breaditnow.customer.adapter.out.persistence.entity.RegionEntity;
...
public interface JpaRegionRepository extends JpaRepository<RegionEntity, String> {
    ...
    @Query("SELECT new com.breaditnow.customer.application.dto.response.SidoResponse(r.sidoCode, r.sidoName) " +
            "FROM RegionEntity r " +
            "GROUP BY r.sidoCode, r.sidoName")
    List<SidoResponse> findSidoResponses();

    @Query("SELECT new com.breaditnow.customer.application.dto.response.GugunResponse(r.sidoName, r.gugunCode, r.gugunName) " +
            "FROM RegionEntity r " +
