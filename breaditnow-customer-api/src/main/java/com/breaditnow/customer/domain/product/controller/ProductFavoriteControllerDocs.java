...
@Tag(name = "Customer - 상품 좋아요 API")
public interface ProductFavoriteControllerDocs {
	@Operation(summary = "고객 ID와 상품 ID를 기반으로 해당 상품에 좋아요(즐겨찾기)를 등록합니다.")
	...
	@Operation(summary = "고객 ID와 빵집 ID를 기반으로 해당 상품에 등록된 좋아요(즐겨찾기)를 삭제합니다.")
	...
	@Operation(summary = "고객 ID, 페이지, 사이즈, 정렬 기준 및 위치 정보를 기반으로 즐겨찾기한 상품 목록을 조회합니다.")
