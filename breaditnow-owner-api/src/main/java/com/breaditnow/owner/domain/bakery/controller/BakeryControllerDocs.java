...
@Tag(name = "Owner - 빵집 API", description = "사업자의 빵집 관리 API입니다. 이 API는 빵집 생성, 조회, 수정, 운영 상태 업데이트, 삭제를 제공합니다.")
public interface BakeryControllerDocs {

	@Operation(
		summary = "빵집 등록",
		description = """
			소유자 정보를 바탕으로 신규 빵집을 등록합니다.<br/>
			요청 데이터는 BakeryCreateRequest(JSON)와 프로필 이미지 파일을 포함하여 multipart/form-data로 전송됩니다.
			"""
	)
	...
	@Operation(summary = "빵집 상세 조회", description = "빵집 ID(bakeryId)를 기반으로 해당 빵집의 상세 정보를 조회합니다.")
	@DomainErrorCodeExamples({BAKERY_NOT_FOUND, BAKERY_INACTIVE})
	@Parameter(name = "bakeryId", description = "조회할 빵집의 ID", example = "1", required = true)
	ApiSuccessResponse<BakeryResponse> getBakery(Long bakeryId);

	@Operation(
		summary = "빵집 정보 수정",
		description = """
			소유자 정보(ownerId)와 빵집 ID(bakeryId)를 이용해 빵집의 정보를 수정합니다.<br/>
			RequestPart("data")로 BakeryUpdateRequest(JSON)을 전송하고, 
			프로필 이미지(profileImage), 추가 이미지 목록(additionalImages)은 multipart로 전송 가능합니다.
			"""
	)
	@Parameter(name = "bakeryId", description = "수정할 빵집의 ID", example = "1", required = true)
	...
	@Operation(summary = "빵집 삭제", description = "특정 빵집을 삭제합니다. 빵집에 관련된 모든 정보가 제거됩니다.")
	@Parameter(name = "bakeryId", description = "삭제할 빵집의 ID", example = "1", required = true)
