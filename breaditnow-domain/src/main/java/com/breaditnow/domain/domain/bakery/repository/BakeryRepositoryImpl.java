...
public class BakeryRepositoryImpl extends QuerydslRepositorySupport implements BakeryRepositoryCustom {
	...
	@Override
	public Page<Bakery> searchHotBakeries(Long customerId, Pageable pageable) {
		...
		query.innerJoin(preference)
			.on(preference.region.id.sidoCode.eq(bakery.address.sidoCode)
				.and(preference.region.id.gugunCode.eq(bakery.address.gugunCode))
				.and(preference.customer.id.eq(customerId)));
		...
		if (pageable.getSort().isSorted()) {
			...
			if ("reservation".equalsIgnoreCase(property)) {
			...
			} else if ("like".equalsIgnoreCase(property)) {
		...
		// 정렬 조건이 없거나 인식되지 않으면 기본은 즐겨찾기 정렬("like")
